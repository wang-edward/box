...
static void assert_tracks(const std::vector<std::unique_ptr<Track>> &tracks,
  ...
  if (tracks.size() == 0 || index >= tracks.size()) {
    throw std::runtime_error{"App::" + name + "(): index [" +
...
void print_tracks(App a) {
  for (auto &t : a.tracks_) {
    // std::cout << "Wave Input Device: " << t->track_.getWaveInputDevice() <<
    // std::endl; WaveInputDevice& getWaveInputDevice() const noexcept        {
    // jassert (waveInputDevice); return *waveInputDevice; } MidiInputDevice&
    // getMidiInputDevice() const noexcept        { jassert (midiInputDevice);
...
Track &App::CurrTrack() { return *tracks_[current_track_]; }

size_t App::GetCurrTrack() { return current_track_; }
    ...
    if (device_type == te::InputDevice::physicalMidiDevice ||
      ...
      if (t != nullptr) {
        [[maybe_unused]] auto res =
            instance->setTarget(t->itemID, true, &edit_.getUndoManager(), 0);
