  ...
  */
  ...
  // render key offset
  {
    ...
    Vector2 text_size =
        MeasureTextEx(GetFontDefault(), text.c_str(), HEADER_FONT_SIZE,
                      HEADER_FONT_SIZE / DEFAULT_FONT_SIZE);
    DrawText(text.c_str(), SCREEN_EIGHTH + SCREEN_EIGHTH / 2 - text_size.x / 2,
  ...
  // render bar lines
  {
    ...
    for (float bar_start = first_bar_start; bar_start < screen.RightEdge();
      ...
      DrawLine(x, HEADER_HEIGHT, x, HEADER_HEIGHT + (ROW_HEIGHT * num_rows),
               DARKGRAY);
    }
  }
  ...
  // render clips
  {
    for (size_t i = 0; i < num_rows; i++) {
      ...
      for (const auto &c : t->base_.getClips()) {
        ...
        BeatWindow clip = {static_cast<float>(t_br.getStart().inBeats()),
        ...
        if (visible_start < visible_end)  // Clip is visible
        {
          ...
          DrawRectangle(left_px, (i * ROW_HEIGHT) + HEADER_HEIGHT, clip_width,
                        ROW_HEIGHT, GREEN);
          DrawRectangleLines(left_px, (i * ROW_HEIGHT) + HEADER_HEIGHT,
  ...
  if (!transport.isRecording() && !transport.isPlaying()) {
    ...
    DrawRectangleLines(left_px, (curr_row * ROW_HEIGHT) + HEADER_HEIGHT, width,
                       ROW_HEIGHT, ORANGE);
  }
...
void Timeline::print_timeline() {
  ...
  for (auto c : clips) {
    ...
    std::cout << "\t\t" << "TIME start: " << time_range.getStart().inSeconds()
              << std::endl;
    std::cout << "\t\t" << "TIME end: " << time_range.getEnd().inSeconds()
              << std::endl;
    std::cout << "\t\t" << "TIME length: " << pos.getLength().inSeconds()
              << std::endl;
    std::cout << "\t\t" << "BEATS start: " << beat_range.getStart().inBeats()
              << std::endl;
    std::cout << "\t\t" << "BEATS end: " << beat_range.getEnd().inBeats()
              << std::endl;
    std::cout << "\t\t" << "BEATS length: " << beat_range.getLength().inBeats()
              << std::endl;
  }
}
              ...
              if (transport.isRecording()) {
...

                {
                  const float curr_pos =
                      APP->edit_.tempoSequence.toBeats(transport.getPosition())
                          .inBeats();
                  cursor_.start =
